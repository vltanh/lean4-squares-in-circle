...
theorem Four.rigid_uniqueness (S : Fin 4 → UnitSquare) (o : Point)
    (hp : Packing S o Four.radius) :
    ∃ (e : Point ≃ Point) (σ : Equiv.Perm (Fin 4)), e (0,0)=o ∧
      (∀ p q, normSq (sub (e p) (e q))=normSq (sub p q)) ∧
      (∀ i p, closedSquare (S (σ i)) (e p) ↔ ClosedRect (Four.centers i) p.1 p.2) :=
  (Four.uniqueness S o hp).rigid_witness
