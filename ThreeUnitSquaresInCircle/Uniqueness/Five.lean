import ThreeUnitSquaresInCircle.Uniqueness.Contacts
...
theorem five_polygon_uniqueness (S : Fin 5 → UnitSquare) (o : Point)
    (hd : InteriorDisjoint S) (hp : ∀ i, P5 (alpha (S i) o) (beta (S i) o)) :
    HasNormalForm S o fiveCenters := by
  classical
...
theorem five_uniqueness (S : Fin 5 → UnitSquare) (o : Point)
    (hp : PackingN S o (Real.sqrt ((5:ℝ)/2))) : HasNormalForm S o fiveCenters := by
  apply five_polygon_uniqueness S o hp.disjoint
  intro i
  apply p5_of_phi_le
  have h := hp.phi_le i
  rwa [Real.sq_sqrt (show (0:ℝ) ≤ 5/2 by norm_num)] at h
