...
lemma lower_strip_fits (p : Point)
    (hx0 : -1 ≤ p.1) (hx1 : p.1 ≤ 1)
    (hy0 : -1 ≤ p.2) (hy1 : p.2 ≤ 0) :
    inDisk tCenter optimalRadius p := by
...
lemma tSquares_contained (i : Fin 3) (p : Point)
    (h : closedSquare (tSquares i) p) : inDisk tCenter optimalRadius p := by
  fin_cases i
...
theorem tSquares_packing : Packing tSquares tCenter optimalRadius := by
  exact ⟨optimalRadius_nonneg, tSquares_contained, tSquares_disjoint⟩

theorem exists_packing_at_optimum :
    ∃ (S : Fin 3 → UnitSquare) (o : Point), Packing S o optimalRadius := by
