...
verifier and three Lean drafts are preserved verbatim in `reference/`; they
play no part in the Lean verification path.

**Occupied-arc proof.** The framework for three, four and five squares was
drafted in [PR #2](https://github.com/vltanh/three-unit-squares-in-circle/pull/2)
and had not been compiled. Claude Opus 5.5 compiled it against Lean
`4.34.0` / mathlib `v4.34.0`. The repairs covered library renames, tactic
normal forms, and the missing measure instance on `Real.Angle`, and no theorem
statement changed. Claude Opus 5.5 then separated the proof from the
certificate modules and made it the main proof.
