...
`four_containing_arc`, `five_containing_arc`, `three_containing_impossible`
(`Containing.lean` in the folders for three to five squares)
...
- **Four squares** (`Four/Containing.lean`). The sweep covers a quarter circle
  on every auxiliary circle of radius below `1/√2`. With three exterior arcs
  longer than `90°`, the budget fails.
...
- **Three squares** (`Three/Containing.lean`). The sweep is too weak, and the
  ...
  1. **Deficit.** On the circle of radius `3/8`, `S` contains an arc of length
     ...
     more than `2π/3`. The three-arc budget forces `L < 2π/3`, and the contact
     tangents bound the deficit `δ = 2π/3 - L < 1/12` (`three_deficit_bounds`).
...
Together with Step 3 this proves `four_polygon_strict_impossible`,
`five_polygon_strict_impossible` and, for every choice of which square contains
`o`, `three_polygon_strict_impossible`.
...
- **Three squares** (`Three/Uniqueness.lean`). A square
  ...
  Step 4 argument still refutes it when the other two squares satisfy only the
