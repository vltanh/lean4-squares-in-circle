#!/usr/bin/env python3
"""No-subdivision checker for A2 P12: W-secondary / D-secondary tail."""
...
vals=[]
for var in ('n','s','e'):
 z=bound(J(N,var=='n'),J(W),J(S,var=='s'),J(E,var=='e'),'Ws').dd
 assert z.hi<0,('Ws',var,z.lo,z.hi);vals.append(z.hi)
for a,b in ((F(0),F(1,20)),(F(1,20),F(1,10)),(F(1,10),F(3,20)),(F(3,20),F(1,5))):
 z=bound(J(N),J(I(a,b),1),J(S),J(E),'Ws').dd
 assert z.hi<0,('Ws','w',a,b,z.lo,z.hi);vals.append(z.hi)
print('Ws max curvature upper',float(max(vals)))
...
assert bestall[0]>F(1,20),bestall
print('worst >',float(bestall[0]),bestall[1:])
print('A2 P12 hand checker PASS')
